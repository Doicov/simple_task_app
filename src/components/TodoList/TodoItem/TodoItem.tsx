...
export const TodoItem: React.FC<TodoItemProps> = ({ todo, onDeleteTodo, onCheckTodo, onEdit }) => (
    <Paper elevation={2} sx={{ maxWidth: '800px', marginTop: '15px', padding: '15px 25px', borderRadius: '15px', 
        display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '15px', opacity: todo.checked ? 0.5 : 1}}>
        <Box textAlign='left'>
            <Typography  
            onClick={() => onCheckTodo(todo.id)}
            sx={{cursor: 'pointer', textDecorationLine: todo.checked ? 'line-through' : 'none'}} variant='h5' component='h5' gutterBottom>
                {todo.name}
            </Typography>
            <Typography sx={{textDecorationLine: todo.checked ? 'line-through' : 'none', maxWidth: "600px"}}variant='subtitle1' component='div' gutterBottom>
                {todo.description}
            </Typography>
        </Box>
        <Box>
